  ...
  return (
    <footer className="relative z-10 bg-gradient-to-br from-blue-50 via-white to-amber-50 pt-14 pb-10 mt-auto border-t border-blue-100 shadow-inner animate-fade-in">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-10 px-4 sm:px-6">
        ...
        {/* Newsletter Signup */}
        <form
          className="flex flex-col sm:flex-row gap-2 items-center bg-white/90 rounded-full px-4 py-2 shadow border border-blue-100 hover:shadow-lg transition mt-6 md:mt-0 w-full sm:w-auto"
          onSubmit={e => { e.preventDefault(); setSubscribed(true); setNewsletter(''); }}
        >
          <input
            type="email"
            className="bg-transparent outline-none px-2 py-1 text-sm text-blue-800 placeholder-blue-300 w-full sm:w-auto"
          ...
          <button
            type="submit"
            className="px-3 py-1 rounded-full bg-rose-600 text-white text-sm font-semibold shadow hover:bg-amber-400 transition disabled:opacity-60 w-full sm:w-auto"
            disabled={subscribed}
          >
            {subscribed ? t('footer_subscribe_success') : t('footer_subscribe_button')}
          </button>
        </form>
      </div>
